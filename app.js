...
// --- ZIKIRMATIK LOGIC ---
let currentZikirMode = 'subhanallah';
let zikirCounts = {
    subhanallah: 0,
    elhamdulillah: 0,
    allahuakber: 0,
...
window.setZikirMode = function(mode) {
    currentZikirMode = mode;
    
    // Update active tab UI
    document.querySelectorAll('.zikir-tab-btn').forEach(btn => {
        btn.style.background = 'var(--bg-card)';
        btn.style.color = 'var(--gold)';
    });
    const activeBtn = document.querySelector(.zikir-tab-btn[onclick="setZikirMode('')"]);
    if(activeBtn) {
        activeBtn.style.background = 'linear-gradient(135deg, #d4af37 0%, #8a6d1c 100%)';
    ...
    const titles = {
        ...
        allahuakber: 'Allahu Ekber',
        serbest: 'Serbest Zikir'
    };
    const titleEl = document.getElementById('zikir-title');
    if(titleEl) titleEl.textContent = titles[mode];
    
    const targetEl = document.getElementById('zikir-target-text');
    if(targetEl) targetEl.textContent = 'Hedef: ' + zikirTargets[mode];
    
    // Update count display
    const countEl = document.getElementById('zikir-count');
    if(countEl) countEl.textContent = zikirCounts[mode];
}

window.incrementZikir = function() {
    if(navigator.vibrate) navigator.vibrate(40);
    
    let count = zikirCounts[currentZikirMode];
    let target = zikirTargets[currentZikirMode];
    
    if (target !== 'Sınırsız' && count >= target) {
        if(navigator.vibrate) navigator.vibrate([100, 50, 100]);
        if(window.showToast) window.showToast("Hedefe ulaşıldı!");
        return;
    }
    
    zikirCounts[currentZikirMode]++;
    const countEl = document.getElementById('zikir-count');
    if(countEl) countEl.textContent = zikirCounts[currentZikirMode];
    
    const btn = document.getElementById('zikir-btn');
    if(btn) {
...
window.resetZikir = function() {
    zikirCounts[currentZikirMode] = 0;
    const countEl = document.getElementById('zikir-count');
